...
import { isDarkThemeActive, navigate } from '../pages/common';
import { getLogoElement, turnOnTheme } from '../pages/header.component';
...
test.describe('Header', { tag: '@header' }, () => {
  ...
  test.describe('Header logo', () => {
    ...
    test('Logo is visible', async ({ page }) => {
      await navigate(page)
      await expect(getLogoElement(page)).toBeVisible()
    });
  })
  ...
  test.describe('Dark and Light theme', () => {
    ...
    test('Switch to the dark theme', async ({ page }) => {
      await navigate(page)
      await turnOnTheme(page, 'dark')
      expect(await isDarkThemeActive(page)).toBe(true)
    });
